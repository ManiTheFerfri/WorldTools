...
object WorldTools {
    ...
    const val ANVIL_EXTENSION = ".mca"
    const val DAT_EXTENSION = ".dat"
    const val MAX_LEVEL_NAME_LENGTH = 64
    const val TIMESTAMP_KEY = "CaptureTimestamp"
    val GSON: Gson = GsonBuilder().setPrettyPrinting().create()
    val CURRENT_VERSION = SharedConstants.getLaunchedVersion().dataVersion().name()
    private val VERSION_SCHEMA: String = LoaderInfo.getVersion()
